    ...
    for enemy in game.enemy_pirates_without_treasures():
        for treasurerer in game.my_pirates_with_treasures():
            if game.in_range(enemy.location, treasurerer.initial_loc):
                if enemy.location == treasurerer.initial_loc:
                    #locations = possible_location(game, pirate, treasurerer.initial_loc, moves)
                    #game.set_sail(pirate, locations)
            ...
            elif game.in_range(enemy.location, treasurerer):
                #locations = possible_location(game, pirate, treasurerer.location, moves)
                #game.set_sail(pirate, locations)
...
def do_treasure(game, pirate):
    if sent_home == False:
        ...
        if not pirate.has_treasure:
            ...
            if len(game.treasures()) == 1 and len(game.enemy_pirates_without_treasures()) != 0 and len(game.enemy_sober_pirates()) != 0:
                return
            if len(game.treasures()) == 0 and len(game.my_pirates_with_treasures()) != 0:
                return
            if (len(game.treasures()) == 1 or len(game.treasures()) == 0) and len(game.enemy_sober_pirates()) != 0:
                do_attack(game, pirate)
                return
            if treasure != False:
...
def try_attack(game, pirate):
    global locked_attacking
    for enemy in game.enemy_sober_pirates():
        if game.in_range(pirate, enemy) and pirate.reload_turns == 0 and enemy not in locked_attacking:
