...
def do_turn(game):
    ...
    if attack_p_up_on_map and p_up != None:
        pirate_to_send = closest_free_pirate_to_powerup(game, p_up)
        if pirate_to_send != False:
            do_powerup(game, pirate_to_send, p_up, give_boost=True)
...
def do_defense(game, pirate):
    for enemy in game.enemy_pirates_without_treasures():
        for treasurerer in game.my_pirates_with_treasures():
            ...
            #    return
            elif game.in_range(enemy, treasurerer) and enemy not in locked_enemies:
...
def do_powerup(game, pirate, power_up, give_boost = False):
    global is_boosted
    global sent_home
...
def try_attack(game, pirate, treasurer = False):
    global locked_attacking
    if not treasurer:
        for enemy in game.enemy_sober_pirates():
        ...
        for enemy in game.enemy_sober_pirates():
            if game.in_range(pirate, enemy) and pirate.reload_turns == 0 and enemy not in locked_attacking and enemy.reload_turns == 0: #and enemy.defense_reload_turns > 0 and not enemy.defense_expiration_turns > 0:
                game.attack(pirate, enemy)
                locked_attacking.append(enemy)
                return True
    return False
...
def assign_roles(game):
    ...
    if len(distances) != 0:
        ...
        while len(go_attackers_ids) < int(len(game.all_my_pirates()) / 2):
            ...
            if pirate1.id not in go_attackers_ids and pirate1.reload_turns == 0 and not pirate1.has_treasure:
...
def possible_location(game, pirate, destination, moves):
    ...
    if len(possible_locations)==0:
        return False#possible_location2(game,pirate,destination,moves)# False
